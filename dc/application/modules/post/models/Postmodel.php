...
//----------------  end---------------
    ...
    function getPosts($ps_status)
    {
        $this->db->select('*');
        $this->db->from('c_posts i');
        $this->db->join('c_users b', 'b.id = i.c_user_id');
        $this->db->join('post_channels c', 'c.post_id = i.post_id');
        $this->db->order_by("i.post_id", "DESC");

        if (!empty($ps_status)) {
            $this->db->where("channel_id", $ps_status);
        }
...
//        print_r($data);
        $arr['post'] = array(
            ...
            "spotlight_link_text" => $data['link_text'],
...
//                    print_r($this->db->error());
    ...
    public function updatePost($data)
    {
        ...
        if (!empty($data['post_html'])) {
            $arr['post']['cp_html'] = $data['post_html'];
        }
        if (!empty($data['cp_post_facebook_image'])){
            $arr['post']['cp_post_facebook_image'] = $data['cp_post_facebook_image'];
        }
        if (!empty($data['post_image'])){  //Logo Image
            $arr['post']['cp_image'] = $data['post_image'];
            $data['cp_image']=$data['post_image'];
        }
        $arr['post']['cp_post_image'] = $data['post_featured_image'];
        $arr['post']['cp_type'] = $data['post_type'];
        $arr['post']["spotlight_link_text"] = $data['link_text'];
//        print_r($data);
//        print_r($arr['post']);
    ...
    public function getPostSpotLightById($postId = NULL)
    {
        $this->db->select('*');
        $this->db->from('post_spotlight');
        $this->db->order_by("id", "DESC");
